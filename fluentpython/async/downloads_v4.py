...
async def download_one(session: aiohttp.ClientSession, cc: str, semaphore: asyncio.Semaphore, verbose: bool = False) -> Result:
    ...
    try:
        ...
        async with semaphore:
            country = await get_country(session, cc)
        ...
        loop.run_in_executor(None, save_flag, filename, image)
...
async def get_country(session: aiohttp.ClientSession, cc: str) -> bytes:
